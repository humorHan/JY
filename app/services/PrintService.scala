 ...
 */
@ImplementedBy(classOf[PrintServiceImpl])
trait PrintService {
...
object PrintService {
  ...
  final case class ProductDetail(id: Int,
    ...
    /**
      * 描述信息的关键词。
      */
    def metaKeywords: String = meta.keywords.getOrElse("")

    /**
      * 描述信息的网页描述。
      */
    def metaDescription: String = meta.description.getOrElse("")
  }
  ...
  /**
    ...
    *
    * @param name 名称。
    * @param image_key 图片七牛key。
    * @param url 跳转链接。
    */
  final case class ProductItem(name: String,
                               image_key: String,
                               url: String) {
    /**
      * 图片完整url。
      */
    def imageUrl(implicit config: Configuration): String = getQiniuUrl(image_key)
  }
  ...
  /**
    ...
    *
    * @param name 名称。
    * @param logo_key logo七牛key。
    * @param description 描述。
    * @param background_key 背景七牛key。
    */
  final case class Custom(name: String,
    ...
    /**
      * logo 的完整url。
      */
    def logoUrl(implicit config: Configuration): String = getQiniuUrl(logo_key)

    /**
      * 背景的完整url。
      */
    def backgroundUrl(implicit config: Configuration): String = getQiniuUrl(background_key)
  }
  ...
  /**
    ...
    * @param custom 客户信息。
    */
  final case class ExampleDetail(id: Int,
                                 title: String,
                                 description: String,
                                 background_key: String,
                                 products: Seq[ProductItem],
                                 designs: Seq[ProductItem],
                                 content: String,
                                 custom: Custom) {
...
final case class Category(name: String, description: String, products: Seq[Product], hide_in_pc: Boolean) {
   ...
   */
  private val PRODUCT_GROUP_LENGTH = 8
}
