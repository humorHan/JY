...
@Singleton
private[services]
class PrintServiceImpl @Inject() (ws: WSClient)
                                 (implicit config: Configuration) extends PrintService {
  import PrintServiceImpl._
  ...
  private implicit val productReads: Reads[services.Product] = Reads { json =>
    Try {
      val coverUrl = {
        // 先从新加的 cover_key 字段获取图片key,获取失败后再尝试从 cover_keys 字段获取第一张商品图片。
        (json \ "cover_key").asOpt[String].orElse {
          (json \ "cover_keys").as[Seq[String]].headOption
        }.map { key =>
          getQiniuUrl(key)
        } getOrElse ""
      }
  ...
  def exampleDetail(id: Int): Future[Option[ExampleDetail]] = {
    val responseFuture = {
      ws.url(s"${config.internalPrintServiceUrl}/api/v2/others/example-detail").
    ...
    responseFuture.map {
      case response if response.status == 200 =>
        if ((response.json \ "result").asOpt[Int].exists(_ != 0)) {
          if (!(response.json \ "result").asOpt[Int].contains(5)) {
            // 案例不存在的情况就不打log了。
            play.api.Logger.error(s"get example detail not SUCCESS, id:$id ${response.json}")
          }

          None
        } else {
          Some(response.json.as[ExampleDetail])
        }

      case _ => None
    }.recover { case ex: Throwable =>
      play.api.Logger.error(s"get example detail error,id:$id", ex)
      None
    }
  }
...
object PrintServiceImpl {
  ...
  private implicit val productItemReads: Reads[ProductItem] = Json.reads[ProductItem]
  private implicit val customReads: Reads[Custom] = Json.reads[Custom]
  private implicit val exampleDetailReads: Reads[ExampleDetail] = Json.reads[ExampleDetail]
