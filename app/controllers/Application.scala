...
class Application @Inject() (implicit config: Configuration,
  ...
  def caseList = defaultPageAction { implicit request => implicit barParam =>
    templates.caseList()
  }

  def caseDetail(id: Int) = defaultPageAction { implicit request => implicit barParam =>
    printService.exampleDetail(id).flatMap {
      case Some(detail) => templates.caseDetail(detail)
      case None => Found("/")
    }
  }
